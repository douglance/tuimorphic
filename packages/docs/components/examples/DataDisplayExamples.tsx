...
import type { TreeNode } from 'tuimorphic';
  ...
  return (
    <div className="space-y-2">
      <TreeView
        ...
        onSelect={(node) => setSelected(node.label)}
      />
      {selected && (
        <p className="text-sm opacity-70">Selected: {selected}</p>
      )}
    </div>
  );
}
...
export function TreeViewDeepNesting() {
  ...
  return (
    <TreeView
      data={data}
      defaultExpanded={['level-1', 'level-2', 'level-3', 'level-4']}
    />
  );
}
...
export function GridCustomGaps() {
  return (
    <Grid columns={2} rowGap={4} columnGap={8}>
      <GridItem>
