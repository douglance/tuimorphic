...
import { Alert, Badge, Progress, BarLoader, BlockLoader, Tooltip, Button, Message, MessageList, } from 'tuimorphic';
// =============================================================================
// Alert Examples
// =============================================================================
...
export function AlertDismissible() {
    ...
    return (<div className="flex flex-col gap-4">
      {alerts.success && (<Alert variant="success" title="Success" dismissible onDismiss={() => setAlerts((prev) => ({ ...prev, success: false }))}>
          Click the X to dismiss this alert.
        </Alert>)}
      {alerts.warning && (<Alert variant="warning" title="Warning" dismissible onDismiss={() => setAlerts((prev) => ({ ...prev, warning: false }))}>
          This warning can also be dismissed.
        </Alert>)}
      {alerts.error && (<Alert variant="error" title="Error" dismissible onDismiss={() => setAlerts((prev) => ({ ...prev, error: false }))}>
          Errors can be dismissed too.
        </Alert>)}
      {!alerts.success && !alerts.warning && !alerts.error && (<Button onClick={resetAlerts}>Reset Alerts</Button>)}
    </div>);
}
// =============================================================================
// Badge Examples
...
export function MessageBasic() {
    return (<Message position="left" author="System">
